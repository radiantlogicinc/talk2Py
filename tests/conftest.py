...
def _copy_directory(src_dir: Union[str, Path], dest_dir: Union[str, Path], exclude: list[str] = None) -> None:
...
def _copy_example_app(app_name: str, tmp_path: Path) -> dict[str, Path]:
    ...
    """
    src_dir = Path(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'examples', app_name)))
    ...
    return {"module_dir": dest_dir, "module_file": module_file, "metadata_path": metadata_path}


@pytest.fixture
def temp_calculator_app() -> Generator[dict[str, Path], None, None]:
...
@pytest.fixture
def temp_todo_app(tmp_path: Path) -> Generator[dict[str, Path], None, None]:
...
@pytest.fixture
def calculator_registry(temp_calculator_app: dict[str, Path]) -> CommandRegistry:
...
@pytest.fixture
def todolist_registry(temp_todo_app: dict[str, Path]) -> CommandRegistry:
...
@pytest.fixture
def calculator_executor(calculator_registry: CommandRegistry) -> CommandExecutor:
...
@pytest.fixture
def todolist_executor(todolist_registry: CommandRegistry) -> CommandExecutor:
...
@pytest.fixture
def chat_context_reset() -> Generator[None, None, None]:
