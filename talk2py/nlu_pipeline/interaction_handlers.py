...
from .models import NLUPipelineContext, InteractionState
# Import the specific data models
from .interaction_models import (
    BaseInteractionData, ClarificationData, ValidationData, FeedbackData
)
...
class InteractionResult:
    ...
    proceed_immediately: bool = False # Should the manager re-run core logic after exiting?
    update_context: Optional[Dict[str, Any]] = field(default_factory=dict) # NLUPipelineContext fields to update
...
class InteractionHandler(ABC):
    ...
    def _get_typed_data(self, context: NLUPipelineContext) -> Optional[BaseInteractionData]:
        ...
        if not context.interaction_data:
            ...
            return None
        if self.expected_data_type and not isinstance(context.interaction_data, self.expected_data_type):
    ...
    @abstractmethod
    def handle_input(self, user_message: str, context: NLUPipelineContext) -> InteractionResult:
...
class ClarificationHandler(InteractionHandler):
    ...
    def handle_input(self, user_message: str, context: NLUPipelineContext) -> InteractionResult:
        data = self._get_typed_data(context)
        if not data or not isinstance(data, ClarificationData):
            return InteractionResult(response="Error processing clarification.", exit_mode=True)
        ...
        try:
            ...
            if 0 <= choice_index < len(data.options):
                ...
                # Invalid number
                 return InteractionResult(response=f"Please enter a number between 1 and {len(data.options)}.\n{self.get_initial_prompt(context)}")
        except ValueError:
            # Input wasn't a number, could add fuzzy matching later
            return InteractionResult(response=f"Please enter the number corresponding to your choice.\n{self.get_initial_prompt(context)}")
...
class ValidationHandler(InteractionHandler):
    ...
    def handle_input(self, user_message: str, context: NLUPipelineContext) -> InteractionResult:
        data = self._get_typed_data(context)
        if not data or not isinstance(data, ValidationData):
             return InteractionResult(response="Error processing validation.", exit_mode=True)
        ...
        if provided_value:
            ...
            updated_params[data.parameter_name] = provided_value # Needs proper type conversion later!
            return InteractionResult(
            ...
            # Empty input
            return InteractionResult(response="Please provide a value.") # Re-prompt implicitly via manager
...
class FeedbackHandler(InteractionHandler):
    ...
    def get_initial_prompt(self, context: NLUPipelineContext) -> str:
        ...
        response_snippet = data.response_text[:200] + "..." if len(data.response_text) > 200 else data.response_text
    ...
    def handle_input(self, user_message: str, context: NLUPipelineContext) -> InteractionResult:
        data = self._get_typed_data(context)
        if not data or not isinstance(data, FeedbackData):
             return InteractionResult(response="Error processing feedback.", exit_mode=True)
