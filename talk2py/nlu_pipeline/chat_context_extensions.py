...
def get_nlu_context() -> NLUPipelineContext:
    ...
    try:
        ...
        if isinstance(context_data.get("current_state"), str):
            try:
                context_data["current_state"] = NLUPipelineState(
                    context_data["current_state"]
                )
            except ValueError:
                logger.warning(
                    "Invalid state value '%s' found in context, resetting state.",
                    context_data["current_state"],
                )
                context_data["current_state"] = (
        ...
        logger.warning("Failed to validate NLU context, resetting: %s", e)
        context = NLUPipelineContext()
        CHAT_CONTEXT.app_context["nlu_pipeline"] = context.model_dump()
        return context
